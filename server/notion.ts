...
export async function getNotionDatabases() {
    if (!NOTION_PAGE_ID) {
        console.error("NOTION_PAGE_ID is not defined");
        return [];
    }
...
// Get blog posts from the Notion database
export async function getBlogPostsFromNotion(databaseId: string, locale = 'en') {
    try {
        ...
        return response.results.map((page: any) => {
            ...
            return {
                ...
                locale: properties.Locale?.select?.name || locale,
                featuredImage: properties.FeaturedImage?.url || "https://via.placeholder.com/800x400",
                author: {
                    name: properties.AuthorName?.rich_text?.[0]?.plain_text || "Anonymous",
                    email: properties.AuthorEmail?.email || "anonymous@example.com"
                }
            };
        });
    } catch (error) {
